...
def cluster(corpus, num_topics, num_clusters):
...
def get_cluster_result(result, corpus, num_topics):
    f = codecs.open('result_kmeans/cluster%d.txt' % day, 'w', encoding='utf-8')
...
def get_top_clusters(corpus_original, num_topics, Threshold):
    f = codecs.open('result_kmeans/cluster%d.txt' % day, 'r', encoding='utf-8')
    data = []
...
def get_result(Threshold, data, result_pair, del_news_id):
    ...
    book.save( 'result_kmeans/%d.xls' % day)
...
if __name__ == '__main__':
    ...
    for day in range(num_of_days):
        ...
        num_of_topics = 200
        average_size_of_a_topic = 50
        num_of_clusters = len(news_list) // average_size_of_a_topic
        cluster_result = cluster(text_ner_corpus, num_of_topics, num_of_clusters)
